...
// The controller method $(id) an element inside the screen with the given id
...
Screens.addController = function (name, controller) {
	var _el = null
	Object.defineProperty(controller, "el", {get: function () {
		if (!_el) _el = document.getElementById("screen-"+name)
		return _el
	}})
	...
	Screens._screens[name] = {inited: false, controller: controller}
}
...
Screens._show = function (name, arg, backwards) {
	...
	if (screen) {
		...
		if (!screen.inited) {
			try {screen.controller.oninit.call(screen.controller)} catch (e) {}
			screen.inited = true
		}
		...
		setTimeout(function () {
			...
			style.left = left+"px"
			setTimeout(function () {
				try {screen.controller.onaftershow.call(screen.controller, arg)} catch (e) {}
			}, 1e3)
		}, 100)
		window.scroll(0, 0)
	}
...
Screens._hide = function (backwards) {
	...
	if (Screens._current) {
		...
		if (screen) {
			style = screen.controller.el.style
			try {screen.controller.onbeforehide.call(screen.controller)} catch (e) {}
